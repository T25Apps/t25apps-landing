      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
          {apps.map((app) => {
            ...
            // --- LIVE APP with external link (RecipeDiary) ---
            if (isLive) {
              return (
                <a
                  key={app.id}
            ...
            // --- IN-DEVELOPMENT APP — show building animation on click ---
            return (
              <button
                key={app.id}
                type="button"
                onClick={() => showBuildingAnimation(app.id)}
                className="group relative bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 dark:border-gray-700 flex flex-col h-full cursor-pointer text-left w-full"
              >
                {/* Status badge */}
                ...
                {/* Icon */}
                <div className={`mb-6 p-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl w-fit text-gray-900 dark:text-white transition-all duration-300 ${isAnimating ? 'animate-bounce' : 'group-hover:scale-110'}`}>
                  {IconComponent && <IconComponent />}
                </div>
                ...
                {isAnimating && (
                  <div className="absolute inset-0 bg-white/95 dark:bg-gray-800/95 rounded-2xl flex flex-col items-center justify-center z-20 animate-fade-in">
